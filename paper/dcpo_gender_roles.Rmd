...
For a detailed description of the DCPO model, see Appendix C and @Solt2020c [, 3-8]; here we focus on how it deals with the principal issues raised by the source data, incomparability and sparsity.
...
For example, we might deduce that strongly disagreeing that "on the whole, men make better business executives than women do" expresses even more egalitarianism than strongly agreeing merely that both spouses should have paid employment.
...
Together, the difficulty and dispersion parameters work to generate comparable estimates of the latent variable of public gender egalitarianism from the available but incomparable source data.
...
That is, within each country, each year's value of public gender egalitarianism is modeled as the previous year's estimate plus some noise.
