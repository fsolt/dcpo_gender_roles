...
We consider now the validity of the PGE Mean scores.
Like @Caughey2019 [684-685], we provide evidence of our new measures' validity with both convergent validation and construct validation tests.
...
```{r convergent, fig.cap="Convergent Validation: Correlations Between PGE Mean Scores and Individual PGE Source Data Survey Items \\label{conv_plot}", fig.height=4}
...
Then, in the center panel, we look at the question that provides most data-rich cross-section.
...
Finally, in the right panel, to evaluate how well the PGE Mean score captures change over time, we focus on the item with the largest number of observations for a single country. 
...
```{r construct_validation_survey, fig.cap="Construct Validation: Correlations Between PGE Mean Scores and Individual 'Balancing' Gender Egalitarianism Survey Items \\label{surv_const_plot}", fig.height=4}
...
In Figure&nbsp;\ref{surv_const_plot}, we turn to construct validation, which refers to demonstrating, for some _other_ concept believed causally related to the concept a measure seeks to represent, that the measure is empirically associated with measures of that other concept [@Adcock2001, 542].
...
Figure&nbsp;\ref{construct_plot} shows additional tests of construct validation.
