...
```{r dcpo_input_raw}
surveys_egr <- read_csv("data-raw/surveys_egr.csv", col_types = "cccc")[1:244,]

dcpo_input_raw <- DCPOtools::dcpo_setup(vars = surveys_egr,
                                        file = "data/dcpo_input_raw.csv")
```

```{r obs_by_item, echo=FALSE, fig.cap = "Items with the Most Observations in the Source Data \\label{items_by_obs}"}
dcpo_input_raw %>%
...
That this constitutes only `r sprintf("%2.1f", (polileader4_cy*100/total_cy))`% of the `r total_cy` total possible country-years covered underscores just how sparse the available public opinion data is on this topic.

```{r obs_by_country, echo=FALSE, fig.cap = "Countries with the Most Observations in the Source Data \\label{countries_by_obs}"}
dcpo_input_raw %>%
...
```{r years_by_country, echo=FALSE, fig.cap="Country-Year Coverage in the Source Data \\label{countries_by_years}"}
library(patchwork)
