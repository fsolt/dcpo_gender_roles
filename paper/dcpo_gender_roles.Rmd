...
knitr::opts_chunk$set(cache = TRUE, echo = FALSE, fig.width=6.5, fig.height=2.5)
...
n_countries <- dcpo_input_raw %>%
  distinct(country) %>% 
  nrow()

year_range <- paste(summary(dcpo_input_raw$year)[6]-summary(dcpo_input_raw$year)[1],
                    "years, from",
                    summary(dcpo_input_raw$year)[1],
                    "to",
                    summary(dcpo_input_raw$year)[6])

n_cyi <- dcpo_input_raw %>% 
...
We draw on data from `r n_surveys` survey datasets, in which we identified `r n_items` distinct relevant survey items.
Together, these survey items were asked in `r n_countries` different countries over `r year_range`, yielding a total of `r n_cyi` country-year-item observations.
...
total_cy <- (max(dcpo_input_raw$year) - min(dcpo_input_raw$year))*(dcpo_input_raw %>% distinct(country) %>% nrow())
...
Figure&nbsp;\ref{items_by_obs} displays in how many country-years each of most-commonly asked items are available.
...
Employed by the Americas Barometer, the Arab Barometer, the Latinobarómetro, the Pew Research Center, and the World Values Survey, this question was asked in a total of `r polileader4_cy` different country-years.
That this constitutes only `r sprintf("%2.1f", (polileader4_cy*100/total_cy))`% of the `r total_cy` total possible country-years covered---and again, `polileader4` is the _most common_ survey item---underscores just how sparse the available public opinion data is on this topic.
...
```{r years_by_country, fig.cap="Country-Year Coverage in the Source Data \\label{countries_by_years}", cache.extra = tools::md5sum(here::here("data", "dcpo_input_raw.csv"))}
library(patchwork)
