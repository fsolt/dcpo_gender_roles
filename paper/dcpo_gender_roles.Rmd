...
We estimated the model on our source data using the `DCPO` package for R [@Solt2020a], running four chains for 4,000 iterations each, discarding the first half as warmup, and thinning the remainder by eight, which left us with 1,000 samples.
...
But neglecting to incorporate this uncertainty by using only the mean estimate for each country-year in an analysis can lead one to mistakenly conclude that the analysis supports the hypothesis [see @Tai2022] as well as to mistakenly conclude that it does _not_ support the hypothesis [see @Crabtree2015].
