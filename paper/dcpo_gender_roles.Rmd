...
This model accounts for the incomparability of different survey questions in two ways.
First, it adjusts for the _difficulty_ of each question's responses, that is, how much egalitarianism is indicated by a given response. 
That different question responses evince more or less of our trait is most easily seen with regard to the ordinal responses to the same question: strongly agreeing with the statement "both the husband and wife should contribute to household income," exhibits more public gender egalitarianism than responding "agree," which in turn is more egalitarian than responding "disagree" or "strongly disagree."
But this is also true across questions.
For example, we might suppose that strongly disagreeing that "on the whole, men make better business executives than women do" expresses even more egalitarianism than strongly agreeing merely that both spouses should have paid employment.
...
Together, the difficulty and dispersion parameters adjust the observed survey data to generate

To address the sparsity of the data---the fact that there are gaps in the source data in the time series of each country, and even many observed country-years have only one or few observed items---DCPO uses simple local-level dynamic linear models (i.e., random-walk priors).
That is, within each country, each year's value of public gender egalitarianism is a function of the previous year's estimate plus some noise.
These dynamic models smooth the estimates of public gender egalitarianism over time and allows estimation even in years for which no survey data is available.
For more detail on the DCPO model, see Appendix C and @Solt2020c [, 3-8].
...
We estimated the model using the `DCPO` package for R [@Solt2020a], running four chains for 4,000 iterations each, with the first half discarded as warmup and the remainder thinned by eight to leave us with 1,000 samples.
