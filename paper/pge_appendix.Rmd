...
Lower values of dispersion ($\alpha$) indicate questions that better identify publics with more public gender egalitarianism from those with less.
Items have one less difficulty score ($\beta$) than the number of response categories.
...
```{r pge_items_table4}    
pge_items_table(24:30)
```

```{r pge_items_table5}    
pge_items_table(31:41)
```

\pagebreak
```{r pge_items_table6}    
pge_items_table(42:50)
```
\vspace{-.95in}
...
dcpo_input_raw_poliwork1 %>% 
  mutate(country = str_replace(country, "’", "'")) %>% 
    ...
    # ggtitle("Source Data Observations by Country and Year")
```
